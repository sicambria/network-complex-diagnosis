...
class TestPingCommand:
    def test_linux_default(self):
        with patch("netdiag_core.runtime.IS_LINUX", True), patch("netdiag_core.runtime.IS_MACOS", False), patch("netdiag_core.runtime.IS_WINDOWS", False):
    ...
    def test_macos(self):
        with patch("netdiag_core.runtime.IS_LINUX", False), patch("netdiag_core.runtime.IS_MACOS", True), patch("netdiag_core.runtime.IS_WINDOWS", False):
            cmd = ping_command("1.1.1.1")
            assert cmd[0] == "ping"
            assert "-t" in cmd

    def test_windows(self):
        with patch("netdiag_core.runtime.IS_LINUX", False), patch("netdiag_core.runtime.IS_MACOS", False), patch("netdiag_core.runtime.IS_WINDOWS", True):
            cmd = ping_command("1.1.1.1")
            assert cmd[0] == "ping"
            assert "-n" in cmd
            assert "-w" in cmd
...
class TestPingOnce:
    def test_success(self):
        with patch("netdiag_core.runtime.run_cmd") as mock_run, \
    ...
    def test_plan_b_tcp_fallback_when_no_ping(self):
        with patch("netdiag_core.runtime.run_cmd") as mock_run, \
             ...
             patch("netdiag_core.runtime.IS_MACOS", False), \
             patch("netdiag_core.runtime.IS_WINDOWS", False), \
             patch("netdiag_core.runtime.has_tool") as mock_has:
